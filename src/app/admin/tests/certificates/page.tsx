...
function TemplateCellRenderer(props: { value: string }) {
    // Strip HTML tags and truncate for display
    const textContent = props.value?.replace(/<[^>]*>/g, '') || '';
    const truncated = textContent.length > 100 ? `${textContent.substring(0, 100)}...` : textContent;
    return (
        <div title={textContent} className="text-gray-700">
            {truncated}
        </div>
    );
}
...
export default function TestCertificatesPage() {
    ...
    const [rows, setRows] = useState<TestCertificateRow[]>([]);
    ...
    const [pendingDelete, setPendingDelete] = useState<TestCertificateRow[]>([]);
    ...
    const columnDefs = useMemo<ColDef<TestCertificateRow>[]>(() => [
        { headerName: "Name", field: "name", cellRenderer: NameCellRenderer, minWidth: 220 },
        { headerName: "Template", field: "template", cellRenderer: TemplateCellRenderer, minWidth: 250 },
    ...
    const buildFilter = useCallback(() => {
        ...
        Object.entries(fm).forEach(([field, cfg]: any) => {
            ...
            const map: Record<string, string> = {
                ...
                template: 'TestCertificateTemplates',
                language: 'Language',
                isActive: 'IsActive',
        ...
        if (query.trim()) filters.push(`contains(TestCertificateName,'${query.trim().replace(/'/g, "''")}')`);
    ...
    const fetchPage = useCallback(async () => {
        ...
        const res = await fetchTestCertificatesAction({ top: pageSize, skip: (page - 1) * pageSize, orderBy, filter });
    ...
    return (
        <div className="p-4 bg-gray-50 h-full min-h-0 flex flex-col">
            <div className="sticky top-0 z-20 bg-gray-50 pt-2 pb-3 flex-none">
                <PageHeader icon={<HelpCircle className="w-6 h-6 text-indigo-600" />} title="Test Certificates" showSearch searchValue={query} onSearch={(v) => { setPage(1); setQuery(v); }} />
            </div>
            <div className="bg-white shadow rounded-lg p-2 flex-1 overflow-hidden flex flex-col min-h-0">
                <div className="mb-3 flex items-center justify-between gap-3 flex-none">
                    <div className="flex items-center gap-3 flex-wrap">
                        <Link href="/admin/tests/certificates/new"><button className="inline-flex items-center gap-2 w-32 px-3 py-2 rounded-md bg-indigo-600 text-white text-sm shadow hover:bg-indigo-700"><PlusCircle className="w-4 h-4" /> New</button></Link>
                        <button
                            disabled={deleting || selectedCount === 0}
                            onClick={() => {
                                const sel = gridApiRef.current?.getSelectedRows?.() as TestCertificateRow[];
                ...
                <div className="ag-theme-alpine ag-theme-evalus w-full flex-1 min-h-0 relative">
                    <AgGridReact<TestCertificateRow>
                        ...
                        overlayNoRowsTemplate="No certificates found"
                        overlayLoadingTemplate=""
                        theme="legacy"
                        animateRows
                    />
                    {loading ? <GridOverlayLoader message="Loading certificates..." /> : null}
                </div>

                <ConfirmationModal
                    isOpen={confirmOpen}
                    title="Confirm Delete"
                    message={`Delete ${pendingDelete.length} selected certificate(s)?`}
                    ...
                    onConfirm={async () => {
                        ...
                        try {
                            for (const row of pendingDelete) {
                                await deleteTestCertificateAction(row.id);
                            }
                            fetchPage();
