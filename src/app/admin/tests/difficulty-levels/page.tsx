...
export default function TestDifficultyLevelsPage() {
  const [query, setQuery] = useState("");
  const [rows, setRows] = useState<TestDifficultyLevelRow[]>([]);
  ...
  const [pendingDelete, setPendingDelete] = useState<TestDifficultyLevelRow[]>([]);
  ...
  const columnDefs = useMemo<ColDef<TestDifficultyLevelRow>[]>(() => [
    { field: 'name', headerName: 'Difficulty Level', minWidth: 220, flex: 1.2, sortable: true, filter: 'agTextColumnFilter', cellRenderer: NameCellRenderer },
  ...
  const fetchPage = useCallback(async () => {
    ...
    const res = await fetchTestDifficultyLevelsAction({ top: pageSize, skip: (page - 1) * pageSize, orderBy, filter });
    if (res.status === 200 && res.data) { setRows(res.data.rows); setTotal(res.data.total); }
    else { setToast({ message: res.message || 'Failed to fetch difficulty levels', type: 'error' }); }
    setLoading(false); firstLoadRef.current = false;
  }, [page, pageSize, query]);

  useEffect(() => { fetchPage(); }, [fetchPage]);

  const formatDate = (val?: string | number) => {
    if (!val) return ""; const d = new Date(val); if (isNaN(d.getTime())) return ""; const dd = String(d.getDate()).padStart(2, "0"); const mm = String(d.getMonth() + 1).padStart(2, "0"); const yyyy = d.getFullYear(); return `${dd}-${mm}-${yyyy}`;
  };

  return (
    <div className="p-4 bg-gray-50 h-full flex flex-col">
      <div className="sticky top-0 z-20 bg-gray-50 pt-2 pb-3">
        <PageHeader icon={<HelpCircle className="w-6 h-6 text-indigo-600" />} title="Test Difficulty Levels" showSearch searchValue={query} onSearch={(v) => { setPage(1); setQuery(v); }} />
      </div>
      <div className="bg-white shadow rounded-lg p-2 flex-1 overflow-hidden flex flex-col min-h-0">
        ...
        <div className="ag-theme-alpine ag-theme-evalus w-full flex-1 min-h-0 relative">
          <AgGridReact<TestDifficultyLevelRow>
        ...
        <ConfirmationModal
          ...
          onConfirm={async () => {
            ...
            try {
              for (const row of pendingDelete) { await deleteTestDifficultyLevelAction(row.id); }
