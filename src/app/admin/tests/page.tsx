...
import { fetchTestsAction, deleteTestAction, type TestRow } from "@/app/actions/admin/tests";
...
function TestsGrid({ query, onClearQuery }: { query: string; onClearQuery?: () => void }) {
  ...
  const columnDefs = useMemo<ColDef<TestRow>[]>(
    () => [
      ...
      { headerName: "S.No.", valueGetter: (p: any) => {
          const idx = (p?.node?.rowIndex ?? 0) as number;
          return idx + 1 + (page - 1) * pageSize;
        }, width: 90, pinned: 'left', sortable: false, filter: false, resizable: false, cellClass: 'no-right-border', headerClass: 'no-right-border' },
  { field: "name", headerName: "Test Name", headerTooltip: "Test Name", sortable: true, filter: 'agTextColumnFilter', filterParams: { buttons: ['apply','reset','clear'] }, width: 300, cellRenderer: NameCellRenderer, tooltipField: "name", cellClass: 'no-left-border', headerClass: 'no-left-border' },
      { field: "startDate", headerName: "Start Date", headerTooltip: "Test Start Date", sortable: true, filter: 'agDateColumnFilter', filterParams: { buttons: ['apply','reset','clear'], browserDatePicker: true }, valueFormatter: ({ value }) => formatDate(value), width: 180 },
      { field: "endDate", headerName: "End Date", headerTooltip: "Test End Date", sortable: true, filter: 'agDateColumnFilter', filterParams: { buttons: ['apply','reset','clear'], browserDatePicker: true }, valueFormatter: ({ value }) => formatDate(value), width: 180 },
  { field: "status", headerName: "Status", headerTooltip: "Is Active", sortable: true, filter: 'agTextColumnFilter', filterParams: { buttons: ['apply','reset','clear'] }, cellDataType: 'text', cellRenderer: (p: any) => (p.value === true ? 'Active' : p.value === false ? 'InActive' : ''), width: 130 },
      { field: "questions", headerName: "Questions", headerTooltip: "Total Questions", sortable: true, filter: 'agNumberColumnFilter', filterParams: { buttons: ['apply','reset','clear'] }, width: 120 },
      { field: "level", headerName: "Level", headerTooltip: "Difficulty Level", sortable: true, filter: 'agTextColumnFilter', filterParams: { buttons: ['apply','reset','clear'] }, width: 160 },
      { field: "candidates", headerName: "Candidates", headerTooltip: "Assigned Candidates", sortable: true, filter: 'agNumberColumnFilter', filterParams: { buttons: ['apply','reset','clear'] }, width: 140 },
      { field: "category", headerName: "Test Category", headerTooltip: "Category", sortable: true, filter: 'agTextColumnFilter', filterParams: { buttons: ['apply','reset','clear'] }, width: 200 },
  { field: "template", headerName: "Test Template", headerTooltip: "Template", sortable: true, filter: 'agTextColumnFilter', filterParams: { buttons: ['apply','reset','clear'] }, minWidth: 220, flex: 1 },
  ...
  const orderBy = sort ? `${fieldMap[sort.colId] ?? "TestStartDate"} ${sort.sort}` : "TestStartDate desc";
    const filters: string[] = [];
    const search = (query ?? "").trim();
    if (search) filters.push(`contains(TestName,'${search.replace(/'/g, "''")}')`);
    ...
    const booleanExpr = (odataField: string, m: any) => {
      ...
      if (v === 'true' || v === 'active' || v === '1' || v === 'yes') b = 'true';
      else if (v === 'false' || v === 'inactive' || v === '0' || v === 'no') b = 'false';
      if (!b) return null;
      const t = (m?.type || (m?.operator ? m?.condition1?.type : undefined)) ?? 'equals';
    ...
    const textExpr = (odataField: string, m: any) => {
      ...
      const build = (cond: any) => {
        ...
        switch (t) {
          case 'equals': return `${odataField} eq '${val}'`;
          case 'notEqual': return `${odataField} ne '${val}'`;
          case 'startsWith': return `startswith(${odataField},'${val}')`;
          case 'endsWith': return `endswith(${odataField},'${val}')`;
          case 'notContains': return `not contains(${odataField},'${val}')`;
          case 'blank': return `${odataField} eq '' or ${odataField} eq null`;
          case 'notBlank': return `${odataField} ne '' and ${odataField} ne null`;
          default: return `contains(${odataField},'${val}')`;
        }
      };
    ...
    const numberExpr = (odataField: string, m: any) => {
      ...
      const build = (cond: any) => {
        ...
        if (cond?.type === 'inRange') {
          ...
          if (isFinite(from) && isFinite(to)) return `${odataField} ge ${from} and ${odataField} le ${to}`;
          return null;
        }
        if (!isFinite(v)) return null;
        switch (t) {
          case 'equals': return `${odataField} eq ${v}`;
          case 'notEqual': return `${odataField} ne ${v}`;
          case 'greaterThan': return `${odataField} gt ${v}`;
          case 'greaterThanOrEqual': return `${odataField} ge ${v}`;
          case 'lessThan': return `${odataField} lt ${v}`;
          case 'lessThanOrEqual': return `${odataField} le ${v}`;
          default: return `${odataField} eq ${v}`;
        }
      };
      if (m.operator && m.condition1 && m.condition2) {
        const a = build(m.condition1);
    ...
    const dateExpr = (odataField: string, m: any) => {
      ...
      const build = (cond: any) => {
        ...
        if (t === 'inRange') {
          ...
          if (from && to) return `${odataField} ge ${from} and ${odataField} le ${to}`;
          return null;
        }
  ...
  const fmStart = fm.startDate ? dateExpr('TestStartDate', fm.startDate) : null;
  const fmEnd = fm.endDate ? dateExpr('TestEndDate', fm.endDate) : null;
  const fmStatus = fm.status ? booleanExpr('IsActive', fm.status) : null;
  const fmQuestions = fm.questions ? numberExpr('TestQuestions@odata.count', fm.questions) : null;
  const fmLevel = fm.level ? textExpr('TestDifficultyLevel/TestDifficultyLevel1', fm.level) : null;
  const fmCandidates = fm.candidates ? numberExpr('TestAssignments@odata.count', fm.candidates) : null;
  const fmCategory = fm.category ? textExpr('TestCategory/TestCategoryName', fm.category) : null;
  const fmTemplate = fm.template ? textExpr('TestTemplate/TestTemplateName', fm.template) : null;
  ...
  const filter = filters.length ? Array.from(new Set(filters)).join(" and ") : undefined;
    ...
    const res = await fetchTestsAction({ top: pageSize, skip: (page - 1) * pageSize, orderBy, filter });
  ...
  <div className="flex items-center gap-3 flex-wrap">
          ...
          <button
            onClick={() => {
              ...
              if (!row) {
                setToast({ message: "Please select a test to delete.", type: "info" });
                return;
              }
        ...
        <div className="mb-3 flex items-center flex-wrap gap-2 flex-none">
          ...
          {Object.entries(filterModelRef.current as Record<string, any>).map(([key, m]) => {
            const nameMap: Record<string,string> = {
              id: 'ID', code: 'Test Code', name: 'Test Name', startDate: 'Start Date', endDate: 'End Date', status: 'Status',
              questions: 'Questions', level: 'Level', candidates: 'Candidates', category: 'Test Category', template: 'Test Template'
            };
            ...
            const shortLabel = (() => {
              ...
              if (c.filterType === 'set' || Array.isArray((m as any)?.values)) {
  ...
  <AgGridReact<TestRow>
          ...
          onFilterChanged={() => {
            const fm = (gridApiRef.current as any)?.getFilterModel?.() as any;
            filterModelRef.current = fm || {};
            setFiltersVersion((v) => v + 1);
