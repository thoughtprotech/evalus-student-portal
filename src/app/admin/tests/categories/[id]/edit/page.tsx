...
export default function EditTestCategoryPage() {
    ...
    const save = async () => {
        ...
        const payload = {
            ...
            createdDate: createdDate ?? nowIso,
            modifiedBy: username,
            modifiedDate: nowIso,
        } as any;
        const res = await updateTestCategoryAction(id, payload);
        setSaving(false);
        ...
        if (ok) { setSaveError(null); setShowSuccessModal(true); }
        else {
            const msg = (res as any)?.errorMessage || (res as any)?.message || 'Update failed';
            setSaveError(typeof msg === 'string' ? msg : 'Update failed');
            try { console.error('Update Test Category failed:', res); } catch { }
        }
    };
    ...
    const ensureTreeLoaded = async () => {
        ...
        try {
            ...
            rows.forEach(r => { if (r.id !== id) { (byParent[r.parentId || 0] ||= []).push({ id: r.id, name: r.name }); } }); // exclude self from being parent
        ...
        (async () => {
            ...
            if (mounted) {
                if (res.status === 200 && res.data) {
                    ...
                    setStatus(Number(res.data.isActive) === 1 ? 1 : 0);
                    setCreatedBy(res.data.createdBy || undefined);
                    setCreatedDate(res.data.createdDate || undefined);
                    if ((res.data.parentId || 0) > 0) {
                        setMode('SUB');
                        setParentCategory({ id: res.data.parentId, name: 'Loading…' });
                        // ensure tree for breadcrumb name
                        await ensureTreeLoaded();
                        const node = idToNode[res.data.parentId];
                        if (node) setParentCategory({ id: node.id, name: node.name });
                    } else {
                        setMode('ROOT');
                    }
                }
        ...
        <div className="p-4 h-full flex flex-col">
            <div className="flex items-start justify-between w-[60%] mx-auto mb-4">
                <div className="flex items-center gap-4">
                    ...
                    <PageHeader icon={<Layers className="w-6 h-6 text-indigo-600" />} title="Edit Test Category" showSearch={false} onSearch={() => { }} />
                </div>
                <div className="flex gap-2">
                    ...
                    <button onClick={save} disabled={!canSave || saving} className="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm font-medium shadow hover:bg-indigo-700 disabled:opacity-50">{saving ? "Saving…" : "Save"}</button>
                </div>
            </div>

            <div className="w-[60%] mx-auto bg-white shadow-sm border border-gray-200 rounded-lg p-6 space-y-5">
                {saveError && (
                    <div className="p-3 rounded-md border border-red-200 bg-red-50 text-sm text-red-700">{saveError}</div>
                )}
                <div className="flex gap-6">
                    ...
                    <label className="inline-flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="text-sm text-gray-800">Sub Category under existing category</span>
                    </label>
                </div>
            ...
            <ConfirmationModal
                ...
                message={mode === 'ROOT' ? 'The category has been updated.' : 'The sub category has been updated under the selected parent.'}
