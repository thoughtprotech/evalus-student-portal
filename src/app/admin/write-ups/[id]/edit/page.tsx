...
export default function EditWriteUpPage() {
  ...
  useEffect(() => {
    if (id === null) {
      setToast({ message: 'Invalid write-up ID', type: 'error' });
      setLoading(false);
      return;
    }

    (async () => {
      const langRes = await fetchLanguagesAction();
      if (langRes.status === 200 && langRes.data) setLanguages(langRes.data);
      const rec = await getWriteUpByIdAction(recordId!);
      if (rec.status === 200 && rec.data) {
        setName(rec.data.name);
        setWriteup(rec.data.writeup);
        setLanguage(rec.data.language);
        setStatus(rec.data.isActive);
        setCreatedBy(rec.data.createdBy);
        setCreatedDate(rec.data.createdDate);
      } else {
        setToast({ message: rec.message || 'Failed to load', type: 'error' });
      }
      setLoading(false);
    })();
  }, [recordId]);
  ...
  const save = async () => {
    ...
    setSaving(true);
    const res = await updateWriteUpAction(recordId!, {
      writeUpName: name.trim(),
      writeUp1: writeup,
      language,
      isActive: status,
