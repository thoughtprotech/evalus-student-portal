...
import { FileText, PlusCircle, Trash2, Filter, XCircle } from "lucide-react";
...
export default function WriteUpsPage() {
  ...
  const [rows, setRows] = useState<WriteUpRow[]>([]);
  const [total, setTotal] = useState(0);
  const [loading, setLoading] = useState(false);
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(15);
  const [showFilters, setShowFilters] = useState(false);
  const [selectedRows, setSelectedRows] = useState<WriteUpRow[]>([]);
  ...
  const columnDefs = useMemo<ColDef<WriteUpRow>[]>(() => [
    ...
    { field: 'writeup', headerName: 'WriteUp', flex: 1, minWidth: 340, filter: 'agTextColumnFilter', sortable: true, valueFormatter: p => {
        const txt = stripHtmlTags(p.value || '').trim();
        if(!txt) return '';
        return txt.length > 140 ? txt.slice(0,137)+'…' : txt;
      }, tooltipValueGetter: p => stripHtmlTags(p.data?.writeup||'') },
  ...
  const fetchPage = useCallback(async (opts?: { silent?: boolean }) => {
    ...
    if (search) filters.push(`contains(WriteUpName,'${search.replace(/'/g,"''")}')`);
    ...
    if (fm.name) { const t = textExpr('WriteUpName', fm.name); if (t) filters.push(t); }
    if (fm.writeup) { const t = textExpr('WriteUp1', fm.writeup); if (t) filters.push(t); }
  ...
  return <div className="p-4 bg-gray-50 h-full flex flex-col">
    <div className="sticky top-0 z-20 bg-gray-50 pt-2 pb-3 flex-none">
      <PageHeader icon={<FileText className="w-6 h-6 text-indigo-600" />} title="WriteUps" showSearch searchValue={query} onSearch={v=>setQuery(v)} />
    </div>
  ...
  {loading && <GridOverlayLoader message="Loading write-ups..." />}
      ...
      `}</style>
      <ConfirmationModal isOpen={confirmOpen} title="Confirm Delete" variant="danger" confirmText={deleting? 'Deleting...':'Delete'} cancelText="Cancel" message={selectedCount>0? (selectedCount===1?`Delete writeup \"${selectedRows[0].name}\"? This cannot be undone.`:`Delete ${selectedCount} writeups? This cannot be undone.`):''} onCancel={()=>{ setConfirmOpen(false); }} onConfirm={async()=>{ if(selectedCount===0) return; setDeleting(true); try { const promises = selectedRows.map(r=>deleteWriteUpAction(r.id)); const results = await Promise.all(promises); const failed = results.filter(r=>r.status!==200); if(failed.length===0){ setToast({ message: selectedCount===1? 'Deleted successfully':'Deleted selected writeups', type:'success'}); } else { setToast({ message: failed.length+ ' deletions failed', type:'error'}); } } finally { setDeleting(false); setConfirmOpen(false); fetchPage(); } }} />
    </div>
