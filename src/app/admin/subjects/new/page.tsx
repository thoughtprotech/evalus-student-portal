...
export default function NewSubjectPage() {
    ...
    return (
        <div className="p-4 space-y-4">
            <PageHeader title="New Subject" icon={<BookOpen className="w-6 h-6 text-indigo-600" />} onSearch={() => { }} showSearch={false} />
            ...
            <div className="bg-white shadow rounded-md p-4 max-w-xl space-y-4">
                <div className="space-y-1"><label className="block text-sm font-medium">Name</label><input value={form.subjectName} onChange={e => setForm(f => ({ ...f, subjectName: e.target.value }))} className="w-full border rounded px-3 py-2 text-sm" placeholder="Subject name" /></div>
                ...
                <div className="space-y-1"><label className="block text-sm font-medium">Parent Id (0 for root)</label><input type="number" value={form.parentId} onChange={e => setForm(f => ({ ...f, parentId: Number(e.target.value) }))} className="w-full border rounded px-3 py-2 text-sm" /></div>
                ...
                <div className="pt-2 flex gap-3"><button disabled={saving} onClick={submit} className="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm disabled:opacity-50">{saving ? 'Saving…' : 'Create'}</button><button onClick={() => router.push('/admin/subjects')} className="px-4 py-2 border rounded-md text-sm">Cancel</button></div>
            </div>
