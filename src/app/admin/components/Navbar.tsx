...
export default function Navbar({ username, userPhoto, companyName, companyLogo }: NavbarProps) {
    ...
    return (
        <header className="w-full border-b border-gray-300 shadow-md px-6 flex items-center justify-between h-16 relative z-50" style={{ paddingLeft: 18 }}>
            {/* Logo */}
            ...
            <nav className="hidden md:flex items-center space-x-6">
                {navItems.filter(n => n.label !== 'Subjects').map(({ label, path, Icon }) => {
                    if (label === 'Questions') {
                        const isActive = pathname.startsWith('/admin/questions') || pathname.startsWith('/admin/subjects') || pathname.startsWith('/admin/write-ups');
...
interface CandidatesSubmenuProps { Icon: React.ComponentType<any>; isActive: boolean; pathname: string; basePath: string; }
function CandidatesSubmenu({ Icon, isActive, pathname, basePath }: CandidatesSubmenuProps) {
    const [open, setOpen] = useState(false);
    const ref = useRef<HTMLDivElement | null>(null);
    const hoverTimeoutRef = useRef<any>(null);
    useEffect(() => {
        ...
        <div ref={ref} className="relative" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
            ...
            {open && (
                <div role="menu" aria-label="Published Documents submenu" className="absolute left-0 top-full bg-white border border-gray-200 rounded-md shadow-lg mt-1 min-w-[260px] z-30 p-1 animate-fade-in">
                    <Link role="menuitem" href="/admin/published-documents/folders" className={`block rounded px-3 py-2 text-sm focus:outline-none focus:bg-indigo-100 ${pathname.startsWith('/admin/published-documents/folders') ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-indigo-50'}`}>Publish Document folder</Link>
