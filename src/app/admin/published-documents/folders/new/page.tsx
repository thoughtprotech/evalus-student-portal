...
import { createPublishedDocumentFolderAction, fetchPublishedDocumentFoldersODataAction } from "@/app/actions/admin/publishedDocumentFolders";
import { BookOpenText, ArrowLeft, ChevronDown, ChevronUp, Circle, Check } from "lucide-react";
import { useUser } from "@/contexts/UserContext";
import { fetchLanguagesAction } from "@/app/actions/dashboard/questions/fetchLanguages";
import type { GetLanguagesResponse } from "@/utils/api/types";

export default function NewPublishedDocumentFolderPage() {
  const router = useRouter();
  const { username } = useUser();

  // Mode selection similar to Candidate Groups (root or sub under parent)
  type Mode = "ROOT" | "SUB";
  ...
  const canSave = useMemo(() => name.trim().length > 0 && language.trim().length > 0 && (mode === 'ROOT' || !!parentFolder), [name, language, mode, parentFolder]);
  ...
  const save = async () => {
    ...
    const payload = {
      id: 0,
      publishedDocumentFolderName: name.trim(),
      parentId: mode === 'ROOT' ? 0 : (parentFolder?.id || 0),
      language: language,
    };
    const res = await createPublishedDocumentFolderAction(payload);
    setSaving(false);
  ...
  // Tree builder from OData list (parentId-based)
  ...
  const parentBreadcrumb = useMemo(() => {
    if (!parentFolder) return ''; const names: string[] = []; let cur: number | null | undefined = parentFolder.id;
    while (cur != null) { const n = idToNode[cur]; if (!n) break; names.push(n.name); cur = idToParent[cur] ?? null; }
    return names.reverse().join(' › ');
  }, [parentFolder, idToNode, idToParent]);
  ...
  const TreeRow = ({ node, level }: { node: Node; level: number }) => {
    ...
    return (
      <div>
        <div data-node-id={node.id} className={`flex items-center gap-2 px-2 py-1 rounded cursor-pointer ${parentFolder?.id === node.id ? 'bg-indigo-50' : 'hover:bg-gray-50'}`} style={{ paddingLeft: level * 12 }}
          onClick={() => { setParentFolder({ id: node.id, name: node.name }); setTreeOpen(false); }} title={node.name}>
          {hasChildren ? (
          ...
          {parentFolder?.id === node.id && <Check className="w-3.5 h-3.5 text-indigo-600 ml-auto" />}
        </div>
    ...
    <div className="p-4 h-full flex flex-col">
      <div className="flex items-start justify-between w-[60%] mx-auto mb-4">
        <div className="flex items-center gap-4">
          ...
          <PageHeader icon={<BookOpenText className="w-6 h-6 text-indigo-600" />} title="New Publish Documents Folder" showSearch={false} onSearch={()=>{}} />
        </div>
      ...
      <div className="w-[60%] mx-auto bg-white shadow-sm border border-gray-200 rounded-lg p-6 space-y-5">
        {/* Hierarchy options first (mimic Candidate Group New) */}
        <div className="flex gap-6">
          <label className="inline-flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm text-gray-800">Create a top-level folder</span>
          </label>
          <label className="inline-flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm text-gray-800">Create a subfolder under an existing folder</span>
          </label>
        </div>
        ...
        {mode === 'ROOT' ? (
          <div>
            ...
            <input value={name} onChange={e=>setName(e.target.value)} className={inputCls} placeholder="Enter folder name" />
          </div>
        ) : (
          <div className="space-y-4">
            <div ref={dropdownRef}>
              <label className="block text-xs font-semibold uppercase tracking-wide text-gray-600">Parent Folder<span className="text-red-500 ml-0.5">*</span></label>
              <button type="button" onClick={async () => { await ensureTreeLoaded(); setTreeOpen(v=>!v); }} className="w-full flex items-center justify-between border border-gray-300 rounded-md px-3 py-2 text-sm bg-white hover:bg-gray-50">
                <span className={`truncate ${parentFolder ? 'text-gray-900' : 'text-gray-500'}`}>{parentFolder ? (parentBreadcrumb || parentFolder.name) : (treeLoading ? 'Loading…' : 'Select a parent folder')}</span>
                ...
                <div className="relative">
                  <div className="absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-md shadow">
                    <div className="p-2 border-b border-gray-100 flex items-center gap-2">
                      <input value={treeFilter} onChange={(e)=>setTreeFilter(e.target.value)} placeholder="Search folders..." className="w-full border border-gray-300 rounded-md px-2 py-1.5 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200" />
                    ...
                    <div ref={scrollRef} className="max-h-72 overflow-auto py-1">
                      {treeLoading ? (
                        ...
                        <div className="p-3 text-sm text-gray-500">No folders found</div>
                      ) : (
                        <div>
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wide text-gray-600">Subfolder Name<span className="text-red-500 ml-0.5">*</span></label>
              <input value={name} onChange={e=>setName(e.target.value)} className={inputCls} placeholder="Enter subfolder name" />
            </div>
          </div>
        )}
      ...
      <ConfirmationModal
        ...
        message={mode === 'ROOT' ? 'The top-level folder has been created.' : 'The subfolder has been created under the selected parent.'}
        confirmText="Go to List"
        cancelText=""
        onConfirm={() => { setShowSuccessModal(false); router.push('/admin/published-documents/folders'); }}
