...
import { createQuestionDifficultyLevelAction } from "@/app/actions/admin/question-difficulty-levels";
import { useUser } from "@/contexts/UserContext";
import type { GetLanguagesResponse } from "@/utils/api/types";

export default function NewDifficultyLevelPage() {
  const router = useRouter();
  const { username } = useUser();
  const [form, setForm] = useState({ questionDifficultylevel1: "", language: "", isActive: 1 });
  ...
  const submit = async () => {
    if (!form.questionDifficultylevel1.trim()) { setToast({ message: 'Name is required', type: 'error' }); return; }
    if (!form.language.trim()) { setToast({ message: 'Language is required', type: 'error' }); return; }
    setSaving(true);
    const nowIso = new Date().toISOString();
    const res = await createQuestionDifficultyLevelAction({
      ...form,
    ...
    <div className="p-4 h-full flex flex-col">
      <div className="flex items-start justify-between w-[60%] mx-auto mb-4">
        <div className="flex items-center gap-4">
          ...
          <PageHeader title="New Difficulty Level" icon={<HelpCircle className="w-6 h-6 text-indigo-600" />} showSearch={false} onSearch={() => { }} />
        </div>
      ...
      <div className="w-[60%] mx-auto bg-white shadow-sm border border-gray-200 rounded-lg p-6 overflow-auto">
        <div className="space-y-5">
          <div className="space-y-1">
            ...
            <input value={form.questionDifficultylevel1} onChange={e => setForm(f => ({ ...f, questionDifficultylevel1: e.target.value }))} className="w-full border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 rounded-md px-3 py-2 text-sm" placeholder="e.g., Easy" />
          </div>
          <div className="space-y-1">
      ...
      <ConfirmationModal
        ...
        message="Your difficulty level has been successfully created."
