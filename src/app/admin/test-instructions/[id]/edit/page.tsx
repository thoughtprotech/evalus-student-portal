...
export default function EditTestInstructionPage(){
  ...
  const save = async ()=>{ if(!canSave) { setToast({ message:'Fill all required fields', type:'warning'}); return; } setSaving(true); const res = await updateTestInstructionAction(recordId, { testInstructionName:name.trim(), testInstruction1: instruction, language, isActive: status }); setSaving(false); if(res.status===200){ setToast({ message:'Instruction updated', type:'success'}); setTimeout(()=> router.push('/admin/test-instructions'), 800); } else setToast({ message: res.message || 'Update failed', type:'error'}); };

  return <div className="p-4 bg-gray-50 h-full flex flex-col">
    <div className="sticky top-0 z-20 bg-gray-50 pt-2 pb-3 flex-none">
      <PageHeader icon={<BookOpenText className="w-6 h-6 text-indigo-600"/>} title="Edit Test Instruction" showSearch={false} onSearch={()=>{}} />
    </div>
    <div className="flex-1 overflow-auto">
      <div className="max-w-4xl mx-auto bg-white shadow rounded-lg p-6 space-y-6">
        <div className="flex items-center gap-3 mb-2"><Link href="/admin/test-instructions" className="inline-flex items-center gap-1 text-sm text-indigo-600 hover:underline"><ArrowLeft className="w-4 h-4"/> Back</Link></div>
        ...
        <div className="pt-4 flex justify-end gap-3">
          <button onClick={save} disabled={!canSave || saving || loading} className="inline-flex items-center gap-2 px-5 py-2 rounded-md bg-indigo-600 text-white text-sm font-medium shadow hover:bg-indigo-700 disabled:opacity-50"><Save className="w-4 h-4"/>{saving? 'Saving...':'Save'}</button>
        </div>
      </div>
    </div>
