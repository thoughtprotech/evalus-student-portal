...
import type { ApiResponse, GetCandidateGroupsInorderResponse } from "@/utils/api/types";
...
export async function fetchCandidateGroupsODataAction(
  params: FetchGroupsParams = { top: 15, skip: 0 }
): Promise<ApiResponse<{ rows: CandidateGroupRow[]; total: number }>> {
  try {
    ...
    sp.set(
      "$select",
      "CandidateGroupId,CandidateGroupName,ParentId,Language,IsActive,CreatedBy,CreatedDate,ModifiedBy,ModifiedDate"
    );
    ...
    if (res.status !== 200 || !res.data) {
      return {
        status: res.status,
        error: true,
        message: res.message || "Failed to fetch candidate groups",
        errorMessage: res.errorMessage,
      };
    }
    const payload: any = res.data;
    const total =
      payload["@odata.count"] ?? payload["@odata.Count"] ?? payload.count ?? 0;
    ...
    return {
      status: 200,
      data: { rows, total },
      message: `Fetched ${rows.length} groups`,
    };
  } catch (e: any) {
    return {
      status: 500,
      error: true,
      message: "Error fetching candidate groups",
      errorMessage: e?.message,
    };
  }
}

export async function fetchCandidateGroupsInorderAction(): Promise<
  ApiResponse<GetCandidateGroupsInorderResponse[]>
> {
  try {
    const res = await apiHandler(endpoints.getCandidateGroupInorder, {});
    return res as any;
  } catch (e: any) {
    return {
      status: 500,
...
export async function createCandidateGroupAction(
  payload: any
...
export async function updateCandidateGroupAction(
  id: number,
  payload: any
): Promise<ApiResponse<any>> {
  try {
    ...
    const res = await apiHandler(endpoints.updateCandidateGroup, {
      id,
      ...body,
    });
    return res as any;
...
export async function deleteCandidateGroupAction(
  id: number
